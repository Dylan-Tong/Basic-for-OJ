...
private:
    struct LINE{
        int line_number;
        string source;
        Statement* Paresed=nullptr;
        LINE(int _num,string _s,bool End=false)
        :line_number(_num),source(move(_s)){};
    };
 ...
 */
   ...
   bool find(list<LINE>::iterator& iter,int line_Number);

   void RUN(int start,EvalState& state);

   void LIST(EvalState& state);

   void CLEAR(EvalState& state);

   void QUIT(EvalState& state);
